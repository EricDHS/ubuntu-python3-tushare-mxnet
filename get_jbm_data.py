...
class myThread (threading.Thread):
    def __init__(self, name, year=None, season=None):
        threading.Thread.__init__(self)
        self.name = name
        self.year = year
        self.season = season
    def run(self):
        logger.info("Starting get_jbm" + self.name)
        year = self.year
        season = self.season
        try:
            if not self.year:
                file_name = 'data/jibenmian/{}.csv'.format(_BASICS_)
                logger.info("Starting thread: {}, to data: {}".format(self.name, file_name))
                ts.get_stock_basics().to_csv(file_name)
